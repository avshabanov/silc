...
static inline silc_obj* silc_internal_get_ref_data(struct silc_mem_t* mem, silc_obj obj, int* subtype, int* len) {
  silc_obj * contents = silc_internal_get_contents(mem, obj);
 ...
 */
static inline int silc_parse_ref(struct silc_mem_t* mem,
                                 silc_obj obj,
                                 int* content_len /* always set if type is reference */,
                                 char** char_content /* set only if bref, otherwise null */,
                                 silc_obj** obj_content /* set only if oref, otherwise null */) {
  int subtype = -1;
  char* pch = NULL;
  silc_obj* po = NULL;
...
static inline silc_obj* silc_get_oref_contents(struct silc_mem_t* mem, silc_obj obj) {
  int content_length = 0;
  silc_obj* result = NULL;
  int subtype = silc_parse_ref(mem, obj, &content_length, NULL, &result);
...
static inline silc_obj* silc_parse_cons(struct silc_mem_t* mem, silc_obj obj) {
  ...
  silc_obj* result = NULL;
  int subtype = silc_parse_ref(mem, obj, &content_length, NULL, &result);
  assert(result != NULL & content_length == 2 && subtype == SILC_CONS_SUBTYPE);
  return result;
}
