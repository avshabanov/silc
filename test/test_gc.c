...
BEGIN_TEST_METHOD(test_get_initial_statistics)
  ...
  silc_internal_init_mem(m, &g_mem_init_empty_root_objs);

  /* Test code goes here */
  struct silc_mem_stats_t stats = {0};

  /* get statistics */
  silc_calc_mem_stats(m, &stats);
...
BEGIN_TEST_METHOD(test_alloc_bref)
  ...
  subtype = silc_parse_ref(m, o2, &content_length, &content, NULL);
  assert(21 == subtype && content_length == strlen(a2) && 0 == memcmp(a2, content, content_length));

  subtype = silc_parse_ref(m, o3, &content_length, &content, NULL);
...
BEGIN_TEST_METHOD(test_gc_partial_cleanup)
  ...
  for (int i = 0; i < countof(objs); ++i) {
    ...
    int len = 0;
    int subtype = silc_parse_ref(m, objs[i], &len, &char_content, &obj_content);
    assert(subtype >= 0);
  ...
  for (int i = 0; i < countof(objs2); ++i) {
    ...
    char* char_content = NULL;
    int len = 0;
    int subtype = silc_parse_ref(m, objs2[i], &len, &char_content, &obj_content);
    assert(subtype >= 0);
    assert((len > 0) && (char_content != NULL || obj_content != NULL));
  }
